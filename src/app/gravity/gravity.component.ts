...
export class GravityComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    for (let i = 0; i < 10; i++) {
      this.ball.push(new Ball(Math.random() * (this.width - this.radious * 2) + this.radious, this.height / 2 * Math.random(), Math.random()*2,1, this.radious));
    }
    this.animation();
  }
  animation() {
    ...
    for (let index = 0; index < this.ball.length; index++) {
      ...
      this.ball[index].update(this.height, this.width);
    }
  }
  resize(event: Event) {
    this.width = window.innerWidth;
